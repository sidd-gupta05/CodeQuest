...
// ---------- Test Catalog ----------
export function TestCatalog({ sampleTestCatalog, inventory, selectedLab, getReagentDetails }: TestCatalogProps) {
  return (
    <Card className="border-gray-100 ">
      <CardHeader>
          ...
          {sampleTestCatalog.map((test) => (
            <Card key={test.id} className="border-l-4 border-gray-300 border-l-blue-500">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">{test.name}</CardTitle>
                  <Badge className="bg-[#F3F6FA] border-gray-100" variant="outline">
                    {test.category}
                  </Badge>
                </div>
                <CardDescription className="font-medium text-[#838FA2]">
