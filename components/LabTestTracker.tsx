...
const TrackReport = () => {
  ...
  const filteredTests = testHistory.filter((test) => {
    const matchesSearch =
      ...
      test.lab.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStatus =
  ...
  // Stepper data - this would normally come from API based on selected test
  const steps = [
    {
      label: 'Test Booked',
      sublabel: 'Today 14:30',
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#178087]"></div>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
            <div className="flex flex-wrap gap-3">
              <select
                ...
                className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-700 bg-white hover:bg-gray-50 transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 text-sm text-[#F36723] bg-orange-50 border border-orange-100 rounded-lg hover:bg-orange-100 transition-colors"
              >
                <RotateCw className="w-4 h-4" /> Reset Filter
              </button>
            </div>
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(4, totalPages) }, (_, i) => {
                  ...
                  if (totalPages > 4 && currentPage > 2) {
                    ...
                    if (pageNum === 4 && currentPage < totalPages - 1)
                      return null;
                  }
                  return (
                    <button
