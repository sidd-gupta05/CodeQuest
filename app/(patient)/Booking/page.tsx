...
import Navbar from '@/components/navbar';
import Footer from '@/components/footer';
// import { labsData } from '@/data/labsData'; // This is no longer needed
import { createSupabaseBrowserClient } from '@/lib/supabase/client'; // Import the Supabase client
import { motion, AnimatePresence } from 'framer-motion';
import Stepper from '@/components/stepper';
import Calendar from '@/components/calendar';
import Link from 'next/link';
...
export default function Booking() {
  ...
  // This useEffect is updated to fetch from Supabase
  useEffect(() => {
    const labId = searchParams.get('labId');
    ...
    const fetchLabData = async () => {
      ...
      // Fetch the lab data including its details and time slots from Supabase
      ...
      if (error || !lab) {
        ...
        setSelectedLab(null); // Clear lab data on error
        return;
      }

      // The data from Supabase needs to be transformed to match the component's expected structure.
      const labDetails = Array.isArray(lab.details)
        ? lab.details[0]
        : lab.details;

      const processedTimeSlots = { Morning: [], Afternoon: [], Evening: [] };
      if (lab.timeSlots) {
      ...
      // Map the database fields to the fields your component uses (e.g., labName -> name)
      const processedLab = {
        id: lab.id,
      ...
      // Set all the states just like before
  ...
  // The loading state and the rest of the component remain unchanged
  if (!selectedLab) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        <p>Loading lab information...</p>
  ...
  // The entire JSX structure remains the same as it was.
  // ===============================================================
  return (
    <>
      <main
