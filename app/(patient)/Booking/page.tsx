...
export default function Booking() {
    ...
    async function generateAvailableTimeSlots(dayOfWeek: string, date: Date) {
        ...
        // Fetch all bookings for this lab & date
        const { data: bookings, error } = await supabase
            .from('bookings')
        ...
        bookings?.forEach(b => {
            if (!b.allocatedEmpId) return; // Only consider bookings with allocated employees
        ...
        while (hour < endHour || (hour === endHour && min < endMin)) {
            ...
            if (allocatedCount < totalEmployees) {
                slots.push(timeStr); // Slot available if not all employees allocated
            }
    ...
    //     loadSlots();
    // }, [selectedDate, labId, totalEmployees, schedules]);
