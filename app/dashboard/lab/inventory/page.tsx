...
const sampleReagentCatalog = [
  { id: 'reagent1', name: 'Hemoglobin Reagent', manufacturer: 'BioTech Labs', category: 'Hematology' },
  { id: 'reagent2', name: 'Buffer Solution pH 7.4', manufacturer: 'ChemCorp', category: 'General' },
  { id: 'reagent3', name: 'PCR Master Mix', manufacturer: 'MolecularTech', category: 'Molecular' },
  { id: 'reagent4', name: 'DNA Extraction Kit', manufacturer: 'GeneticSolutions', category: 'Molecular' },
  { id: 'reagent5', name: 'Urine Dipstick Solution', manufacturer: 'DiagnosticPlus', category: 'Clinical Chemistry' },
  { id: 'reagent6', name: 'Enzyme Substrate', manufacturer: 'BioEnzymes Inc', category: 'Clinical Chemistry' }
];
...
const Inventory = () => {
  ...
  const getStockStatus = (quantity: number, threshold: number, expiryDate: string) => {
    const today = new Date();
    const expiry = new Date(expiryDate);
    const daysToExpiry = Math.ceil((expiry.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    
    if (daysToExpiry <= 0) return { status: 'expired', color: 'destructive', icon: XCircle };
    if (daysToExpiry <= 7) return { status: 'expiring', color: 'destructive', icon: AlertCircle };
    if (quantity <= 0) return { status: 'out-of-stock', color: 'destructive', icon: XCircle };
    if (quantity <= threshold) return { status: 'low-stock', color: 'secondary', icon: AlertTriangle };
  ...
  const filteredInventory = inventory.filter(item => {
    ...
    const matchesCategory = filterCategory === 'all' || reagent?.category === filterCategory;
  ...
  const executeTest = async (testId: string) => {
    ...
    const updatedInventory = inventory.map(item => {
      const reagentUsage = test.reagents.find(r => r.reagentId === item.reagentId);
      ...
      <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 bg-red-500 text-white flex items-center justify-center text-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-white border-[#F7F8F9]">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-sm font-[500px]">Total Reagents</CardTitle>
              <Package className="h-4 w-4 text-muted-foreground font-[#F7F8F9]" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-light">{totalReagents}</div>
              <p className="text-xs font-normal text-[#64748B] text-muted-foreground">Active inventory items</p>
            </CardContent>
          </Card>

          <Card className="bg-white border-[#F7F8F9]">
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-sm font-[500px]">Low Stock Alerts</CardTitle>
              <AlertTriangle className="h-4 w-4 text-orange-500" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-light text-orange-600">{lowStockCount}</div>
              <p className="text-xs font-normal text-[#64748B] text-muted-foreground">Require reordering</p>
            </CardContent>
          </Card>

          <Card className='bg-white border-[#F7F8F9]'>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-sm font-[500px]">Expiring Soon</CardTitle>
              <Clock className="h-4 w-4 text-red-500" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-light text-red-600">{expiringCount}</div>
              <p className="text-xs font-normal text-[#64748B] text-muted-foreground">Within 7 days</p>
            </CardContent>
          </Card>

          <Card className='bg-white border-[#F7F8F9]'>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-sm font-[500px]">Tests Available</CardTitle>
              <TestTube className="h-4 w-4 text-green-500" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-light text-green-600">{sampleTestCatalog.length}</div>
              <p className="text-xs font-normal text-[#64748B] text-muted-foreground">Ready to execute</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="inventory" className="space-y-6">
          <TabsList className="grid w-full grid-cols-5 bg-[#F1F5F9] text-[#64748B]">
            <TabsTrigger      className="data-[state=active]:bg-white data-[state=active]:border data-[state=active]:border-[#F7F8F9] data-[state=active]:text-black"
            value="inventory">Inventory</TabsTrigger>
            <TabsTrigger      className="data-[state=active]:bg-white data-[state=active]:border data-[state=active]:border-[#F7F8F9] data-[state=active]:text-black"
            value="tests">Test Catalog</TabsTrigger>
            <TabsTrigger      className="data-[state=active]:bg-white data-[state=active]:border data-[state=active]:border-[#F7F8F9] data-[state=active]:text-black"
            value="execute">Execute Tests</TabsTrigger>
            <TabsTrigger      className="data-[state=active]:bg-white data-[state=active]:border data-[state=active]:border-[#F7F8F9] data-[state=active]:text-black"
            value="custom">Custom Reagents</TabsTrigger>
            <TabsTrigger      className="data-[state=active]:bg-white data-[state=active]:border data-[state=active]:border-[#F7F8F9] data-[state=active]:text-black"
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className='text-[#64748B] font-normal'>Manage stock levels and track expiry dates</CardDescription>
                  </div>
                  <Dialog >
                    ...
                    <DialogContent className={`${interFont.className} bg-white border-[#F7F8F9] max-w-lg`}>
                      ...
                      <div className="space-y-4 border-[#F7F8F9]">
                        <div>
                          <Label className="pb-1 text-md" htmlFor="reagent">Reagent</Label>
                          <Select >
                            <SelectTrigger className='border-[#dbdcdd] w-full p-4 py-5 text-md' >
                              <SelectValue className='border-[#F7F8F9] text-lg' placeholder="Select reagent" />
                            </SelectTrigger>
                            <SelectContent className='bg-white border-[#F7F8F9] shadow-sm'>
                              {sampleReagentCatalog.map(reagent => (
                                <SelectItem className="hover:bg-gray-200 text-sm pl-3 " key={reagent.id} value={reagent.id}>
                                  {reagent.name}
                                </SelectItem>
                              ))}
                            </SelectContent>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <Label className="pb-1 text-md" htmlFor="quantity">Quantity</Label>
                            <Input className="p-4 py-5 text-md border-[#dbdcdd]" id="quantity" type="number" placeholder="100" />
                          </div>
                          <div>
                            <Label className="pb-1 text-md" htmlFor="unit">Unit</Label>
                            <Select>
                              <SelectTrigger className="border-[#dbdcdd] p-4 py-5 text-md">
                                <SelectValue className='p-4 py-5 text-md' placeholder="ml" />
                              </SelectTrigger>
                        ...
                        <div>
                          <Label className="pb-1 text-md" htmlFor="batch">Batch Number</Label>
                          <Input className='border-[#dbdcdd] p-4 py-5 text-md' id="batch" placeholder="BATCH2024001" />
                        </div>
                        {/* <div>
                          <Label htmlFor="location">Storage Location</Label>
                          <Input id="location" placeholder="Fridge A-1" />
                        </div> */}
                        <Button className="w-full bg-black text-white cursor-pointer hover:bg-gray-800">Add to Inventory</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                </div>
                ...
                <div className="flex items-center space-x-4 mb-6">
                  ...
                  <Select value={filterCategory} onValueChange={setFilterCategory}>
                    ...
                    <SelectContent className='border-[#e8e8e9] bg-white'>
                      <SelectItem className='hover:bg-gray-200' value="all">All Categories</SelectItem>
                      <SelectItem className='hover:bg-gray-200' value="Hematology">Hematology</SelectItem>
                      <SelectItem className='hover:bg-gray-200' value="Molecular">Molecular</SelectItem>
                      <SelectItem className='hover:bg-gray-200' value="Clinical Chemistry">Clinical Chemistry</SelectItem>
                      <SelectItem className='hover:bg-gray-200' value="General">General</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                    ...
                    <TableBody>
                      {loading ? (
                        ...
                        filteredInventory.map((item) => {
                          const reagent = getReagentDetails(item.reagentId);
                          const stockStatus = getStockStatus(item.quantity, item.reorderThreshold, item.expiryDate);
                          const StatusIcon = stockStatus.icon;
                          
                          return (
                            <TableRow className='hover:bg-[#F8FAFC] border-[#e8e8e9]' key={item.id}>
                              ...
                              <TableCell className="font-mono text-sm font-black">{item.expiryDate}</TableCell>
                      ...
                      <CardContent>
                        <div className="space-y-3">
                          ...
                          {test.reagents.map((reagent, index) => {
                            ...
                            const available = inventoryItem ? inventoryItem.quantity >= reagent.quantity : false;
          ...
          <TabsContent value="custom"
            ...
            <Card className='border-gray-100'>
              ...
              <CardContent>
                <div className="space-y-4">
                  {customReagents.filter(reagent => reagent.labId === selectedLab).map((reagent) => {
                    const stockStatus = getStockStatus(reagent.quantity, reagent.reorderThreshold, reagent.expiryDate);
                    const StatusIcon = stockStatus.icon;
                        ...
                        <CardContent>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <p>{reagent.reorderThreshold} {reagent.unit}</p>
                            </div>
                            {/* <div>
                  ...
                  {customReagents.filter(reagent => reagent.labId === selectedLab).length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      <Package className="h-12 w-12 mx-auto mb-4 opacity-50" />
                      <p>No custom reagents added yet</p>
                      <p className="text-sm">Add lab-specific reagents using the button above</p>
                    </div>
                  )}
                </div>
              </CardContent>
