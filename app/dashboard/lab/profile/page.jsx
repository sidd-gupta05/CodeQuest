...
const LabForm = () => {
  ...
  //     } = await supabase.auth.getUser();
  //     if (user) {
  //       const { data: labId } = await supabase
  //         .from('labs')
  //         .select('id')
  //         .eq('id', user?.id)
  //         .single();
  //     }
  //     setLabId(labId);
  //     console.log("Fetched labId:", labId);
  //   }
    ...
    async function fetchUser() {
      ...
      if (user) {
        ...
        if (error) {
          ...
          setLabId(data.id);
          // console.log("Fetched labId:", data.id);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    async function fetchLabData() {
      ...
      const { data, error } = await supabase
        .from("lab_details")
        .select("*")
        .eq("labId", labId)   // make sure this matches Lab.id
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        labName: formData.labName,
        testType: formData.testType || null,
        // nextAvailable: formData.timeSlots.filter((s) => s.date && s.time),
        experienceYears: formData.experienceYears
          ? parseInt(formData.experienceYears)
          : null,
  ...
  return (
    ...
    >
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-2xl">
        ...
        {message && (
          <div
            className={`mt-6 p-4 rounded-md flex items-center ${isError
