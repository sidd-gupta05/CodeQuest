...
export default function VerifyPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Get the stored signup payload from sessionStorage
      const savedPayload = sessionStorage.getItem('signupPayload');
      if (!savedPayload) {
      ...
      // Prepare verification payload with OTP code
      const verifyPayload = {
        ...
        role: payload.role || 'PATIENT', // Default to PATIENT if not specified
      };
      ...
      if (!res.ok) {
        ...
        console.log('Success:', data);
        // Clear the stored payload after successful verification
